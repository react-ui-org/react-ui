...
Responsive spacing utility classes enable to simply add or modify `margin` and
`padding` properties where necessary.

## Naming System

The classes are named using the format `[property][sides]-[size]` for `xs` and
`[property][sides]-[breakpoint]-[size]` for `sm`, `md`, `lg`, `xl`, `xxl`, and
...
Next: [Typography →](/utilities/typography)
