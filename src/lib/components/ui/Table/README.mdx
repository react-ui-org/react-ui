...
To use the Table component, you need to import it first:
...
- Tables are **good for displaying complex tabular data.** For simpler data sets
  or even plain key-value pairs, consider using the [List](/components/ui/list)
  component.
...
it's better for performance to get the paged and sorted data right from the
server rather than loading and processing it all in the browser afterwards.
...
Next: [TextArea →](/components/ui/text-area)
