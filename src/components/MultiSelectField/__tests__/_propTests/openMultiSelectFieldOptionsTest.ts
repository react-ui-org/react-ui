...
export const openMultiSelectFieldOptionsTest: PropTests = [
  {
  ...
  {
    name: 'options:shape[defaultOpened] & size:string=small',
    onBeforeSnapshot: async (page, component) => {
      await component.getByRole('combobox').click();
      await page.waitForTimeout(500);
    },
  ...
  {
    name: 'options:shape[defaultOpened] & size:string=large',
    onBeforeSnapshot: async (page, component) => {
      await component.getByRole('combobox').click();
      await page.waitForTimeout(500);
    },
  ...
  {
    name: 'options:shape[defaultOpened] & searchAlgorithm:null',
    onBeforeSnapshot: async (page, component) => {
      await component.getByRole('combobox').click();
      await page.waitForTimeout(500);
    },
    props: { searchAlgorithm: null },
  },
  {
    name: 'options:shape[selectedOptionFocused]',
    onBeforeSnapshot: async (page, component) => {
      await component.getByRole('combobox').click();
      // Focus the first option which is selected by default
  ...
  {
    name: 'options:shape[searchFiltered]',
    onBeforeSnapshot: async (page, component) => {
      await component.getByRole('combobox').click();
  ...
  {
    name: 'options:shape[searchWithoutResults]',
    onBeforeSnapshot: async (page, component) => {
      await component.getByRole('combobox').click();
