...
export const ScrollViewForTest = ({
  children,
  ...props
}: ScrollViewTestProps) => (
  <div
...
export const ScrollViewForDetectEndAutoscrollTest = ({
  ...
  const [scrollViewContent, setScrollViewContent] = React.useState(baseText);
  ...
  return (
    ...
    >
      <button
        onClick={() => {
          setScrollViewContent(`${scrollViewContent}${generateRandomString()}`);
        }}
        type="button"
      >
        Add text
      ...
      <ScrollView
        autoScroll="detectEnd"
        direction="vertical"
        id="scrollbar"
        {...props}
      >
        <div key={scrollViewContent.length}>
          {scrollViewContent}
        </div>
      </ScrollView>
    </div>
