import type { TransferPropsInvalidProp } from './transferProps.types';
...
] as const satisfies readonly TransferPropsInvalidProp[];

// Declared locally so that consumers compiling the sources directly do not need `@types/node`.
 ...
 */
export const transferProps = <Props extends object>(props: Props): Omit<Props, TransferPropsInvalidProp> => {
  const {
    children,
    className,
    contentEditable,
    ...
    ...restProps
  } = props as Props & Record<TransferPropsInvalidProp, unknown>;

  if (process.env.NODE_ENV !== 'production') {
    const invalidProps = INVALID_PROPS
